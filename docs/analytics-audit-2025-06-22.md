...
Проект: `dmc-lk-client`  
Область: аналитика (события p/v/d/i), дашборд-виджеты, журнал инцидентов, группировка этикеток.

---

## Резюме

Модуль прошёл серию улучшений (React Query, кэш, skeleton, инциденты), но остаются **критические** проблемы с отключёнными count-виджетами и **системные** проблемы с двойными запросами в labels/type/itog-set/events-defect. Lazy-loading виджетов аналитики фактически не работает из‑за eager-импорта в `app/widgets.tsx`.

**Рекомендуемый порядок работ:** Critical → High (производительность/данные) → High (UX) → Medium → Low.
...
Начать **Этап 7** — восстановить count-виджеты (`return ""`) и прогнать smoke-тест дашборда.
