...
export async function requestAnalytics(params: IRequestAnalytics): Promise<IResponse<IResponseAnalytics>> {
	const arr = []
	for (let key in params) {
		if (Array.isArray(params[key as keyof IRequestAnalytics])) {
			;(params[key as keyof IRequestAnalytics] as string[]).forEach(item => {
