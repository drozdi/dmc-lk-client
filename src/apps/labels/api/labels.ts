...
export async function requestLabelsList({ size = 100, number = 0 }: { size?: number; number?: number }) {
...
export async function requestLabelsAddFormat({ format, production_id }: { format: string; production_id: number }) {
