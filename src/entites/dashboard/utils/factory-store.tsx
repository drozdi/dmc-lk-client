...
function dashboardFactory ({
	...
	varibles = {},
	key = "i",
}: FactoryDashboardStoreProps): StateCreator<WidgetContextType> {
	return (set, get, api) => ({
			key,
			widgets: [],
			layouts: [],
			...
			removeWidget: (widget) => {
				...
				set((state) => ({
					...
					layouts: state.layouts.filter((l) => l[key as keyof ILayoutItem] !== widget.id),
				}));
			},
			updateWidget: (widget, layout = {}) => {
				...
				set((state) => ({
					...
					layouts: state.layouts.map((item) =>
						item[key as keyof ILayoutItem] === widget.id ? { ...item, ...layout } : item,
					),
				}));
			},
			renderWidget: (widget) => {
	...
	return create<WidgetContextType>(
		dbMiddleware<WidgetContextType>(
			...
			}),
			{
				...
				default: {},
				load: async (key): Promise<Partial<WidgetContextType> | undefined> => {
