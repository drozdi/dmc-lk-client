import { useGrouped, useStoreCountLabel, useStoreLabels } from '@/entites/labels'
	...
	const ddata = useMemo(() => {
		...
		for (const his of storeCountLabel.history) {
			...
			for (const fp in res[his.production_id]) {
				format = res[his.production_id][fp]?.find(e => e.print === his.format_template)?.format || '.default'
				if (format !== '.default') {
					break;
				}
			}
			if (res[his.production_id][format]) {
				const label = res[his.production_id][format]?.find(e => e.print === his.format_template)
				if (label) {
		...
		<Loading active={storeCountLabel.isLoading} keepMounted>
			<Accordion>
				 {Object.entries(ddata).map(([production_id, item] )=> (
					<Accordion.Item key={production_id} value={`tab-${production_id}`}>
						<Accordion.Control>{qp.findNameById(Number(production_id))}</Accordion.Control>
						<Accordion.Panel>
							<Table>
								<Table.Tbody>
									<Table.Tr>
									...
									{item['.default'].map(item => (
										<Table.Tr key={item.print}>
											...
											<Table.Td>{item.consumption_m}</Table.Td>
										</Table.Tr>
									))} 
									<Table.Tr>
										<Table.Th colSpan={2}>
									...
									{Object.entries(item).map(([label, item])  => {
										if (label === '.default') {
											return
										}
										return <Table.Tr key={label}>
												<Table.Td>{label}</Table.Td>
												<Table.Td>{


												(item || []).reduce((acc, item) => acc + item.consumption_m, 0)
