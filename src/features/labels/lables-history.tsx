...
export const LabelsHistory = () => {
	...
	const placeHistory = useMemo<ICountLabelHistoryItem[]>(() => {
		return dataHistory.filter(
			(item) => place === 0 || item.place_id === place,
		);
	}, [dataHistory, place]);

	const SelectPrint = factorySelect(
		useMemo<factorySelectProps>(
			() => ({
				...
				dataSelect: [
					...
					[...new Set(placeHistory.map((item) => item.format_template))].map(
						(value) => ({
							value,
							label: value,
						}),
	...
	const data = useMemo<ICountLabelHistoryItem[]>(() => {
		return placeHistory.filter(
			(item) => !print || item.format_template === print,
		);
	}, [placeHistory, print]);

	useEffect(() => {
		storeCountLabel.loadHistory(true, apiFilterdate);
	}, [apiFilterdate.join("|")]);

	const handleFilterdate = (value: [DateValue, DateValue]) => {
		if (value[0] && value[1]) {
			setFilterdate(value);
		}
	};
		...
		<Loading active={storeCountLabel.isLoading} keepMounted>
			<Stack>
				<Group justify="space-between" grow>
					...
					<DualCalendarRange value={filterdate} onChange={handleFilterdate} />
					<Group miw={320} justify="space-between" flex="0" grow>
						<SelectPlace
							label="Линия"
							value={String(place)}
							onChange={(v) => setPlace(Number(v))}
						/>
						<SelectPrint
							label="Этикетка"
							value={print}
							onChange={(v) => setPrint(v ?? "")}
						/>
					</Group>
				</Group>
				<TableData<ICountLabelHistoryItem>
					...
					layout={({ nodes, columns }) => (
						<SimpleGrid cols={1}>
							{nodes.map((item) => (
								<Card key={item.index} withBorder>
									{columns
											...
											>
												<div>
													...
													<Text opacity={0.6} fz="xs">
														{dayjs(item.data.date_applic).format(
															$setting.get("formatDateTime"),
														)}{" "}
														- {item.data.format_template}
													</Text>
												</div>
												<Stack>
													<Text fw={500}>{item.data.consumption_m || "-"} м.</Text>
													<Text fw={500}>{item.data.count_label || "-"} шт.</Text>
												</Stack>
											</Group>
										))}
								</Card>
				...
				>
					<DataColumn<ICountLabelHistoryItem>
						field="place_name"
						body={(item) => (
							<>
								<Text truncate="end">{item.place_name}</Text>
								<Text opacity={0.6} fz="xs">
									{dayjs(item.date_applic).format(
										$setting.get("formatDateTime"),
									)}{" "}
					...
					<DataColumn<ICountLabelHistoryItem>
						field="consumption_m"
						body={(item) => <>{item.consumption_m || "-"} м.</>}
					/>
					<DataColumn<ICountLabelHistoryItem>
						field="count_label"
						body={(item) => <>{item.count_label || "-"} шт.</>}
					/>
				</TableData>
				{storeCountLabel.historyHasMore && (
					<Group justify="center">
						<Button
							variant="light"
							loading={storeCountLabel.isLoading}
							onClick={() => storeCountLabel.loadMoreHistory()}
						>
							Загрузить ещё
						</Button>
					</Group>
